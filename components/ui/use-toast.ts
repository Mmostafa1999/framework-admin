...
function toast({ ...props }: Toast) {
  ...
  const existingToast = memoryState.toasts.find(toast => 
    toast.title === props.title && 
    toast.description === props.description
  );
  
  // If a similar toast exists and is still open, don't create a new one
  if (existingToast && existingToast.open) {
    return {
      id: existingToast.id,
